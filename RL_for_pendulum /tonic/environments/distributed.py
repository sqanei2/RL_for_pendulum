...
class Parallel:
    ...
    def initialize(self, seed):
        def proc(action_pipe, index, seed):
            ...
            while True:
                actions = action_pipe.recv()  # recv() will block until something arrives. https://docs.python.org/3.9/library/multiprocessing.html#multiprocessing.connection.Connection
    ...
    def stop_all_processes(self):
        for process in self.processes:
            if process.is_alive():
                process.terminate()  # force stop the process, because there is a while-true loop, it won't stop by itself.
                process.join()  # wait until it is stopped.
                process.close()  # close and release all resources, this will finally remove the process from system monitor.
        print("#Hao: all processes have been terminated, joined, then closed.")
